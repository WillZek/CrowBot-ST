...
//if (chat.delete) return conn.sendMessage(m.chat, { text: '🏷 *Hey!! el anti link esta activo pero eres admin, ¡salvado!*' }, { quoted: m }) 
if (!isBotAdmin) return conn.sendMessage(m.chat, { text: '🌼 *No soy admin, no puedo eliminar intrusos*', mentions: [...groupAdmins.map(v => v.id)] }, { quoted: m });
if (isBotAdmin) {
if (m.text.includes(linkThisGroup)) return
await conn.sendMessage(m.chat, { text: '📎 *¡Enlace detectado!*\n\n*${await this.getName(m.sender)} mandaste un enlace prohibido por lo cual seras eliminado*', mentions: [m.sender] }, { quoted: m })    
