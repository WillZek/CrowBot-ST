...
handler.before = async (m, { conn }) => {
    ...
    try {
        if (text === '🎶' || text === 'audio') {
            await conn.reply(m.chat, 'Iniciando descarga de audio...', fkontak, m || null);
            ...
            await conn.reply(m.chat, 'Iniciando descarga de video...', fkontak, m || null);
...
async function ytMp3(url) {
    return new Promise((resolve, reject) => {
        ytdl.getInfo(url).then(async (getUrl) => {
            ...
            let thumb = getUrl.player_response.microformat.playerMicroformat
