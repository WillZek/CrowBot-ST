...
let handler = async (m, { conn, args }) => {
  try {
    ...
    txt += `❄️  *Nombre* : ${json.name}\n`;
    txt += `❄️  *Visitas* : ${json.watchers_count}\n`;
    txt += `❄️  *Peso* : ${(json.size / 1024).toFixed(2)} MB\n`;
    txt += `❄️  *Actualizado* : ${moment(json.updated_at).format('DD/MM/YY - HH:mm:ss')}\n`;
