const handler = async (m, {conn, text, participants, isAdmin, isOwner, usedPrefix, command}) => {
  ...
  const fproducto = {key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(false ? {remoteJid: '17608914335@s.whatsapp.net'} : {})}, message: {'productMessage': {'product': {'productImage': {'mimetype': 'image/jpeg', 'jpegThumbnail': imagen1}, 'title': `ᴄᴏᴍᴜɴɪᴄᴀᴅᴏ ᴏғɪᴄɪᴀʟ ᴀ ɢʀᴜᴘᴏs`, 'description': 'Hola', 'currencyCode': 'USD', 'priceAmount1000': '1000000000', 'retailerId': 'Ghost', 'productImageCount': 1}, 'businessOwnerJid': `0@s.whatsapp.net`}}};
  if (!m.quoted) return m.reply(`*[🌠] Responde a un mensaje con el comando para enviar el aviso.*`);
  ...
  m.reply(`*[🌠] Mensaje enviado a: ${groups.length} chat(s).*\n\n_*💛.-NOTA:* Posiblemente el comando presente algunas fallas, y el aviso no sea enviado a todos los chats. Disculpe el inconveniente._`);
};
handler.help = ['bcgc2'];
handler.tags = ['owner'];
